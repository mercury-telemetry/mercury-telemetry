...
class UtilityFunctionsTest(TestCase):
    ...
    def test_createOrResetBuiltInSensorTypeAtPresetIndex(self):
        ...
        for index in range(totalTestSensorTypes):
            ...
            sensorType = AGSensorType.objects.get(pk=expected_sensor_type["agSensorTypeID"])

            self.assertEqual(sensorType.name, expected_sensor_type["agSensorTypeName"])
            self.assertEqual(
                sensorType.processing_formula, expected_sensor_type["agSensorTypeFormula"]
            )
            self.assertEqual(sensorType.format, expected_sensor_type["agSensorTypeFormat"])

            # test object (sensor type) reset

            sensorType.name = expected_sensor_type["agSensorTypeName"] + " "
            sensorType.processing_formula = expected_sensor_type["agSensorTypeFormula"] + 1
