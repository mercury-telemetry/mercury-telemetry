...
class TestErrorLogModels(TestCase):
    test_error_log_data = {
        ...
        "invalid_error": {
            ...
            "error_raw_data": r'{date":"2020-03-01 02:25:13.112234","sensor_id":"100",\
    ...
    def test_error_log_table(self):
        for error in self.test_error_log_data.keys():
            foo = ErrorLog.objects.get(
                error_timestamp=self.test_error_log_data[error]["error_timestamp"]
            )
            self.assertEqual(
                foo.error_timestamp, self.test_error_log_data[error]["error_timestamp"]
            )
            self.assertEqual(
            ...
            self.assertEqual(
                ...
                self.test_error_log_data[error]["error_description"],
            )
            self.assertEqual(
                foo.error_raw_data, self.test_error_log_data[error]["error_raw_data"]
            )
