...
class FormulaPipelineTests(TestCase):
    ...
    def test_no_event(self):
        ...
        backup_agactiveevent = AGActiveEvent.objects.all()
        backup_event = self.pipeline.event

        AGActiveEvent.objects.all().delete()
        self.pipeline.event = None
        self.pipeline.save_measurement(sensor, self.timestamp, {"volumetricFlow": 2.0})
        ...
        for act_event_obj in backup_agactiveevent:
            act_event_obj.save()
